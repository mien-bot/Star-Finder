...
export function featuresToSvg(
  ...
  options?: { showOsmAttribution?: boolean }
): string {
  ...
  // OSM Attribution
  const osmAttribution = options?.showOsmAttribution ? `
  <!-- OSM Attribution -->
  <text x="2" y="${vbH - 2.5}" font-size="1.1" fill="#888" font-family="Arial, sans-serif">Data: © OpenStreetMap contributors</text>
  ` : '';

  // Render legend if there are multiple feature types
  const featureTypes = [...new Set(features.map(f => f.type))];
  const legendHeight = featureTypes.length * 4 + 5;
  const legendY = vbH - legendHeight;
  const legendSvg = featureTypes.length > 1 ? `
  <!-- Legend -->
  <g transform="translate(${vbW - 16}, ${legendY})">
    <rect width="14" height="${legendHeight}" fill="white" fill-opacity="0.85" stroke="#ccc" stroke-width="0.1" rx="0.5"/>
    <text x="7" y="2.5" font-size="1.4" fill="#333" text-anchor="middle" font-weight="600">Legend</text>
